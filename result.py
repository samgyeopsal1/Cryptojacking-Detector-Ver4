...
class FinalResult:
    ...
    def extract_from_clovax(self):
        ...
        # 결과 요약 메시지 및 위험도 레벨 저장
        if len(self.extracted) >= 1:
              self.clovax_summary = f"AI 기반 정적 분석에서 {len(self.extracted)}개의 위험 요소가 탐지됨"
              self.clovax_level = "warn" if len(self.extracted) < 3 else "danger"
        else:
              self.clovax_summary = "AI 기반 정적 분석에서 위험 요소가 탐지되지 않음"
    ...
    def combine_results(self, cnt):
        ...
        if len(self.extracted)>=1 and cnt>=1:
            ...
            return "AI + Signature 기반에서 모두 위험 요소가 감지됨", "danger"
